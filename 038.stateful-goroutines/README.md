...
這次改利用 channel block 的特性，來達成像是 mutex 同步的效果
...
func main() {
	...
	go func() {
		...
		for {
			select {
			case read := <-reads:    // 當 reads 沒東西就會 block
				read.resp <- state[read.key]
			case write := <-writes:  // 當 writes 沒東西就會 block
		...
		go func() {
			for {
				read := &readOp{
					key:  rand.Intn(5),
					resp: make(chan int)}
        // 當 reads 裡面還有 data 就會 block
				reads <- read
        // output read 裡面的 resp channel 裡面的資料
				<-read.resp
		...
		go func() {
			for {
				write := &writeOp{
					...
					val:  rand.Intn(100),
					resp: make(chan bool)}
        // 當 writes 裡面還有 data 就會 block
				writes <- write
        // output weite 裡面的 resp channel 裡面的資料
				<-write.resp
